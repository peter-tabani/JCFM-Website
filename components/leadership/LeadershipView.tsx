...
const COLORS = ["bg-[#1e3a5f]", "bg-[#2563eb]", "bg-[#16a34a]", "bg-[#1e293b]", "bg-[#b45309]"];
          ...
          <div className="mb-12 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            <div className="grid lg:grid-cols-[340px_1fr]">
              <div className={`${colorFor(0)} flex flex-col items-center justify-center gap-5 px-8 py-12 text-center`}>
                ...
                <div>
                  {director.tag && <span className="mb-2 inline-block rounded-full bg-[#d97706] px-4 py-1 text-xs font-bold uppercase tracking-wider text-white">{director.tag}</span>}
                  <h2 className="hero-title text-2xl text-white">{director.name}</h2>
                  <p className="mt-1 text-sm text-white/50">{director.role}</p>
                </div>
                {director.email && (
                  <a href={`mailto:${director.email}`} className="flex w-full items-center justify-center gap-2 rounded-full border border-white/20 px-4 py-2.5 text-xs font-semibold text-white/70 transition hover:border-[#d97706] hover:text-[#d97706]">
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className={`${colorFor(0)} flex items-center gap-4 px-5 py-5`}>
              ...
              <div className="flex-1 min-w-0">
                {director.tag && <span className="mb-0.5 inline-block rounded-full bg-[#d97706] px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-white">{director.tag}</span>}
            ...
            <div className="px-5 py-4">
              ...
              <button onClick={() => setActive({ leader: director, color: colorFor(0) })} className="rounded-full bg-[#d97706] px-4 py-2 text-xs font-bold text-white">Read More</button>
            </div>
          </div>
          ...
          {team.map((member, idx) => (
            <div key={member.name} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              ...
              <div className="px-5 py-4">
                ...
                <button onClick={() => setActive({ leader: member, color: colorFor(idx + 1) })} className="rounded-full border border-[#d97706] px-4 py-2 text-xs font-bold text-[#d97706]">Read More</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mx-auto max-w-[1400px]">
          <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-wrap gap-3">
              <a href="mailto:excellentkenya@gmail.com?subject=Teaching Position Enquiry" className="inline-flex items-center gap-2 rounded-full bg-[#d97706] px-6 py-3 text-sm font-bold text-white transition hover:bg-[#b45309]">
