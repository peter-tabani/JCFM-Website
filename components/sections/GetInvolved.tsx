...
export default function GetInvolved() {
  return (
    <section id="get-involved" className="border-b border-slate-200 bg-white">
      ...
      >
        <div className="absolute inset-0 bg-gradient-to-r from-[#0b2545]/85 to-[#4c1d95]/60" />
      ...
      <div className="mx-auto max-w-[1400px] px-5 py-12 sm:px-6 md:py-20">
        ...
        <div className="grid grid-cols-1 divide-y divide-slate-200 border-y border-slate-200 md:grid-cols-3 md:divide-x md:divide-y-0">
          {DOORS.map((d) => (
            <div key={d.title} className="group flex flex-col p-8 transition hover:bg-white md:p-10">
              ...
              <h3 className="mt-2 font-serif text-2xl font-semibold leading-snug text-[#4c1d95]">
                {d.title}
              </h3>
              ...
              <Link
                href={d.link.href}
                className="mt-6 inline-flex w-fit items-center gap-2 border-b border-[#4c1d95] pb-1 text-[11px] font-bold uppercase tracking-[0.22em] text-[#4c1d95] transition hover:border-[#dc2626] hover:text-[#dc2626]"
              >
                {d.link.text} →
              </Link>
            </div>
