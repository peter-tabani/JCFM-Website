...
/* ──────────────────────────────────────────────────────
   ...
   Pastors, Office). Email/password only — no Google sign-in,
   since this console reaches sensitive donor/member data and
   shouldn't be unlockable by any matching Gmail account.
   Passwords are verified against a salted hash (see
   lib/admin.ts + scripts/hash-password.mjs), not stored here.
   ────────────────────────────────────────────────────── */
    ...
    <div className="grid min-h-screen grid-cols-1 bg-[#f8f6ee] lg:grid-cols-[1.1fr_1fr]">
      ...
      <main className="flex items-center justify-center px-5 py-10 sm:px-8 sm:py-14 lg:py-10">
        <div className="w-full max-w-[440px]">
          ...
          {/* Error banner */}
          {err && (
            <div className="mt-6 flex items-start gap-3 border-l-4 border-[#a8201a] bg-[#a8201a]/5 px-4 py-3 text-[13px] text-[#a8201a]">
          ...
          {/* Credentials form */}
          <form onSubmit={onSubmit} className={`space-y-5 ${err ? "mt-5" : "mt-6"}`}>
                <div>
          ...
          <p className="mt-8 text-center text-[11px] text-slate-500">
            ...
            <a
              href="mailto:coordinator@jcfm.online"
