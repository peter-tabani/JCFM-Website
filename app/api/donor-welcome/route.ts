...
export async function POST(req: Request) {
  try {
    ...
    if (!rawName || !rawEmail || rawName.length > 100 || !EMAIL_RE.test(rawEmail)) {
      return NextResponse.json({ success: false, error: "Invalid name or email" }, { status: 400 });
    }
